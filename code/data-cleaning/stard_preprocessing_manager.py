...
def select_rows(input_dir_path):
    ...
    for filename in os.listdir(input_dir_path):
        ...
        if scale_name in ["ccv01"]:
            ...
            if scale_name == "ccv01":
                criteria_2_df = scale_df[
                    (scale_df["level"] == "Level 1") & (2 <= scale_df["week"]) & (scale_df["week"] < 3)]
            ...
            # Special case, hcdm01 is a modified version of dm01. To avoid changing all downstream naming,
            # just re-assign.
            scale_name = "dm01"
        ...
        # Handles creating the preliminary file. See end of this function to see how qids was split up.
            ...
            print("\nNumber of qids week column are null after selecting empty qvtot rows: {}".format(
                sum(scale_df["week"].isnull())))

            # Handle filling in week with 0's and 2's
            week_zero_cond = (scale_df["week"].isnull()) & (scale_df["days_baseline"] < 8) & (
                    scale_df["level"] == "Level 1")
            week_two_cond = (scale_df["week"].isnull()) & (scale_df["days_baseline"] < 22) & (
                    scale_df["level"] == "Level 1")

            scale_df.loc[week_zero_cond, "week"] = 0
            print("Number of qids week column are null after selecting conditions for filling with 0: {}".format(
                sum(scale_df["week"].isnull())))

            scale_df.loc[week_two_cond, "week"] = 2
            print("Number of qids week column are null after selecting conditions for filling with 2: {}".format(
                sum(scale_df["week"].isnull())))
...
def select_subject_rows(scale_df, scale_name, selection_criteria, debug=False):
    ...
    for subjectkey, subject_rows_df in subject_group:
        ...
        # There could be multiple matches, so select a single one based on the selection criteria configuration for a
        # scale.
        ...
        if scale_name == "dm01" and np.isnan(condition_val):
            # This is hard-coded, because it was added later and it's unknown whehter the desired effect for this
            # scale is shared with all the other scales. Would need to spend more time to evaluate this, and compare
            # before/after row selection cases for each scale. For now, keep it this way. Reason: all values for the
            # "enroll" selection criteria condition are blank.
            col_matches_condition_val = (scale_df[selector_col_name] ==
                                         condition_val) | (np.isnan(scale_df[selector_col_name]))
...
def aggregate_rows(root_data_dir_path):
    ...
    for i, filename in enumerate(os.listdir(input_dir_path)):
        ...
        if i == 0:
            ...
            # With an outer join (as opposed to left join) this is likely not relevant anymore but I'm leaving this
            # as I don't have time to check this. It will not cause any issues anyway.
    ...
    aggregated_df = aggregated_df.reindex(
        columns=(main_keys + list([a for a in aggregated_df.columns if a not in main_keys])))
...
def impute(root_data_dir_path):
    ...
    for i, filename in enumerate(os.listdir(input_dir_path)):
        ...
        for i, row in agg_df.iterrows():
            if ('gender||F' in row and 'gender||M' in row) and (
                    np.isnan(row['gender||F']) or np.isnan(row['gender||M'])):
                ...
                if len(gender_series) == 0:
                    ...
                    print("This subject [" + row[
                        'subjectkey'] + "] does not have a value stored in 'crs01' for gender. Keep this blank for now.")
                else:
            ...
            if 'interview_age' in row and np.isnan(row['interview_age']):
                ...
                if len(age_series) == 0:
                    # Ultimately want to remove subjects that have age or gender missing, likely.
                    print("This subject [" + row[
                        'subjectkey'] + "] does not have a value stored in 'crs01' for interview_age. Keep this blank "
                                        "for now.")
                else:
            ...
            if 'qids01_w0sr__qstot' in row:
                value = np.nanmax(
                    list(row[["qids01_w0sr__vsoin", "qids01_w0sr__vmnin", "qids01_w0sr__vemin", "qids01_w0sr__vhysm"]])) \
                        + np.nanmax(
                    list(row[["qids01_w0sr__vapdc", "qids01_w0sr__vapin", "qids01_w0sr__vwtdc", "qids01_w0sr__vwtin"]])) \
                        + np.nanmax(list(row[["qids01_w0sr__vslow", "qids01_w0sr__vagit"]])) \
                        + np.sum(row[["qids01_w0sr__vmdsd", "qids01_w0sr__vengy", "qids01_w0sr__vintr",
                                      "qids01_w0sr__vsuic", "qids01_w0sr__vvwsf", "qids01_w0sr__vcntr"]])
                agg_df.at[i, 'qids01_w0sr__qstot'] = value
            if 'qids01_w2sr__qstot' in row:
                value = np.nanmax(
                    list(row[["qids01_w2sr__vsoin", "qids01_w2sr__vmnin", "qids01_w2sr__vemin", "qids01_w2sr__vhysm"]])) \
                        + np.nanmax(
                    list(row[["qids01_w2sr__vapdc", "qids01_w2sr__vapin", "qids01_w2sr__vwtdc", "qids01_w2sr__vwtin"]])) \
                        + np.nanmax(list(row[["qids01_w2sr__vslow", "qids01_w2sr__vagit"]])) \
                        + np.sum(row[["qids01_w2sr__vmdsd", "qids01_w2sr__vengy", "qids01_w2sr__vintr",
                                      "qids01_w2sr__vsuic", "qids01_w2sr__vvwsf", "qids01_w2sr__vcntr"]])
                agg_df.at[i, 'qids01_w2sr__qstot'] = value
            if 'qids01_w0c__qstot' in row:
                value = np.nanmax(
                    list(row[["qids01_w0c__vsoin", "qids01_w0c__vmnin", "qids01_w0c__vemin", "qids01_w0c__vhysm"]])) \
                        + np.nanmax(
                    list(row[["qids01_w0c__vapdc", "qids01_w0c__vapin", "qids01_w0c__vwtdc", "qids01_w0c__vwtin"]])) \
                        + np.nanmax(list(row[["qids01_w0c__vslow", "qids01_w0c__vagit"]])) \
                        + np.sum(row[["qids01_w0c__vmdsd", "qids01_w0c__vengy", "qids01_w0c__vintr",
                                      "qids01_w0c__vsuic", "qids01_w0c__vvwsf", "qids01_w0c__vcntr"]])
                agg_df.at[i, 'qids01_w0c__qstot'] = value
            if 'qids01_w2c__qstot' in row:
                value = np.nanmax(
                    list(row[["qids01_w2c__vsoin", "qids01_w2c__vmnin", "qids01_w2c__vemin", "qids01_w2c__vhysm"]])) \
                        + np.nanmax(
                    list(row[["qids01_w2c__vapdc", "qids01_w2c__vapin", "qids01_w2c__vwtdc", "qids01_w2c__vwtin"]])) \
                        + np.nanmax(list(row[["qids01_w2c__vslow", "qids01_w2c__vagit"]])) \
                        + np.sum(row[["qids01_w2c__vmdsd", "qids01_w2c__vengy", "qids01_w2c__vintr",
                                      "qids01_w2c__vsuic", "qids01_w2c__vvwsf", "qids01_w2c__vcntr"]])
                agg_df.at[i, 'qids01_w2c__qstot'] = value

        # Re-iterate through agg_df for imputation of the new features, so that "row" contains updated values,
        # to fix imput_qidscpccg bug
        for i, row in agg_df.iterrows():
...
def add_new_imputed_features(df, row, i):
    imput_anyanxiety = ['phx01__psd', 'phx01__pd_ag', 'phx01__pd_noag', 'phx01__specphob', 'phx01__soc_phob',
                        'phx01__gad_phx']
    ...
    imput_maier = ['hrsd01__hmdsd', 'hrsd01__hvwsf', 'hrsd01__hintr', 'hrsd01__hslow', 'hrsd01__hpanx', 'hrsd01__heng',
                   'hrsd01__hagit']
    df.at[i, 'imput_maier'] = np.sum(row.reindex(imput_maier))

    imput_santen = ['hrsd01__hmdsd', 'hrsd01__hvwsf', 'hrsd01__hintr', 'hrsd01__hslow', 'hrsd01__hpanx', 'hrsd01__heng',
                    'hrsd01__hsuic']
    df.at[i, 'imput_santen'] = np.sum(row.reindex(imput_santen))

    imput_gibbons = ['hrsd01__hmdsd', 'hrsd01__hvwsf', 'hrsd01__hintr', 'hrsd01__hpanx', 'hrsd01__heng',
                     'hrsd01__hsuic', 'hrsd01__hagit', 'hrsd01__hsanx', 'hrsd01__hsex']
    df.at[i, 'imput_gibbons'] = np.sum(row.reindex(imput_gibbons))

    imput_hamd7 = ['hrsd01__hmdsd', 'hrsd01__hvwsf', 'hrsd01__hintr', 'hrsd01__hpanx', 'hrsd01__hsanx',
                   'hrsd01__ hengy', 'hrsd01__hsuicide']
    ...
    imput_idsc5w0 = ['qids01_w0c__vmdsd', 'qids01_w0c__vintr', 'qids01_w0c__vengy', 'qids01_w0c__vvwsf',
                     'qids01_w0c__vslow']
    val_imput_idsc5w0 = np.sum(row.reindex(imput_idsc5w0))
    df.at[i, 'imput_idsc5w0'] = val_imput_idsc5w0

    imput_idsc5w2 = ['qids01_w2c__vmdsd', 'qids01_w2c__vintr', 'qids01_w2c__vengy', 'qids01_w2c__vvwsf',
                     'qids01_w2c__vslow']
    ...
    val = round((row['qids01_w2c__qstot'] - row['qids01_w0c__qstot']) / row['qids01_w0c__qstot'] if row[
        'qids01_w0c__qstot'] else 0, 3)
...
def check_qlesq_criteria(df):
    ...
    for subject, group in group_df:
        ...
        assert group[
                   'totqlesq'].isna().sum() == 0, f"Total Qlesq has {group['totqlesq'].isna().sum()} NA values for {subject}"
...
def generate_qlesq_y(root_data_dir_path, output_name, days_baseline_cutoff=77):
    ...
    for subject_id, data in group_qlesq:
        ...
        if "Level 2" in sorted_data.level.values:
            sorted_data = sorted_data[sorted_data[
                                          'level'] != 'Follow up']  # this protects us from using a follow up value for someone who has been in level 2 for some time
        ...
        assert sorted_data[
                   'totqlesq'].isna().sum() == 0, f"Total Qlesq has {sorted_data['totqlesq'].isna().sum()} NA values for {subject_id}"
        ...
        assert sorted_data[sorted_data['level'] == 'Level 2'].shape[
                   0] < 2, f"More than 2 level 2 entries used for {subject_id}"  # make sure we only keep 1 level 2 entry
        assert sorted_data[sorted_data['level'] == 'Follow up'].shape[
                   0] < 2, f"More than 2 level 2 entries used for {subject_id}"
...
def select_subjects(root_data_dir_path):
    ...
    ##test  
    X_77_qlesq_sr__final = handle_subject_selection_conditions(input_row_selected_dir_path, X_tillwk4_qids_sr,
                                                               y_qlesq_77, 'sr')
    X_91_qlesq_sr__final = handle_subject_selection_conditions(input_row_selected_dir_path, X_tillwk4_qids_sr,
                                                               y_qlesq_91, 'sr')
...
def handle_subject_selection_conditions(input_row_selected_dir_path, X, y_df, qids_version):
    ...
    # Select subjects that have ucq entries, aka eliminate subjects that don't have ucq entries, as a proxy for the
    # small amount of subjects missing most patients.
